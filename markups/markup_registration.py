...
async def inline_ntrp_quest(chat_id):
    ...
    title_btn_yes = await title.load_title(chat_id, 'reg_ntrp_btn_yes')
    title_btn_no = await title.load_title(chat_id, 'reg_ntrp_btn_no')
    # way 1
...
async def inline_ntrp_accept(chat_id):
    ...
    # way 2
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=await title.load_title(chat_id, 'reg_ntrp_btn_accept'),
                              callback_data='reg_ntrpyes')]])
    return keyboard
    # return keyboard_builder.adjust(2).as_markup()


async def registration_temp(chat_id):
    title_btn = await title.load_title(chat_id, 'reg_temp')

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=title_btn,
                              callback_data=f'reg-temp')],
    ])
